...
| --- | --- | --- | --- |
| Thumbtack | Pay-per-lead | Easy onboarding | $40+/lead, no escrow, low close rate |
| Angi (HomeAdvisor) | Pay-per-lead, sells the same lead to multiple pros | Marketing scale | Same lead sold 3-4 times, race-to-bottom auctions |
| TaskRabbit | Marketplace with 15% take + percentage to platform | Bookable like a hotel | No back-office, dominated by Apple/IKEA partnerships |
...
- **The structural problem:** Their entire monetization model is selling cold
  leads to multiple pros at $20–$80 each, regardless of conversion. This
  guarantees pro dissatisfaction (~70% of leads convert to zero work). The
  classic "everyone has a Thumbtack story" of the contractor who quit after
  spending $1,500 in a month with one job to show for it.
- **Why we win:** We charge $0 to bid, and only take a fee when a job actually
  closes. We are the structurally opposite model.

### Angi (HomeAdvisor / Angie's List)

- **Strength:** Brand recognition from the merger.
- **The structural problem:** They sell the same homeowner's job to 3-4 pros at
  once and race-to-the-bottom them. Customer experience is being bombarded with
  phone calls within 60 seconds of posting. Pro experience is "show up fast or
  lose."
- **Why we win:** Stoop jobs go to *all* qualified pros in the area but as bids,
  not phone calls. Homeowners review bids on their own time. The customer-pro
  interaction is asynchronous and asynchronous is a feature.

### TaskRabbit

- **Strength:** Beloved for assembly + small tasks. Apple/IKEA partnerships.
- **The structural problem:** Optimized for micro-tasks ($60 IKEA assembly), not
  trade work. Doesn't capture license/insurance verification. Doesn't have the
  business-OS depth that a working plumber needs.
- **Why we win:** We start with licensed trade work first and add the back-office
  features (scheduling, invoicing) that real pros need to stay.
...
- **The structural problem:** Zero infrastructure. No payments. No verification.
  No recourse. Cash-only handshake deals.
- **Why we win:** We layer escrow + verification + invoicing on top of the
...
- **The structural problem:** Centered on full-renovations and high-end pros, not
  small/medium maintenance jobs.
...
- **The structural problem:** They don't bring customers. Pros still need leads
  from Thumbtack/Angi/word-of-mouth.
- **Why we win:** We bring the customer *and* the back-office, in one tool, in
  one bill. Over time we converge on Jobber's depth while owning the demand
  side.

### Angi Services / Handy

- **Strength:** Bookable like ordering an Uber — fixed prices, instant booking.
...
- A renovation marketplace (Houzz / Sweeten own that).
...
compounded — it's not a feature, it's an accumulation.

The narrow geographic launch is what makes this moat-buildable in the first place.
...
- **Verification + insurance compliance is more expensive than we model.**
  Likely. Budget accordingly in v0.3. Don't compromise.
- **Pros find our review feed unfavorable and refuse to participate.** Real risk.
  Mitigation: clear rules, dispute path, and a fair "respond to a review"
  affordance that doesn't compromise reviewer signal.
